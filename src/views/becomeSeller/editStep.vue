    ...
    <div class="add-shop-right-container">
      ...
      <div class="form-box">
        <el-form :model="addShopForm" label-width="150px" :rules="addShopFormRules" ref="addShopForm">
          ...
          <el-form-item>
            <el-button class="add-btn" @click="handleOpen"> 确定</el-button>
          </el-form-item>
        </el-form>
      </div>
    </div>
...
import { getMarchantDetail, updateMarchant } from '@/api/becomeSeller.js'
  ...
  methods: {
    ...
    async getMarchantDetail () {
      try {
        ...
        if (+res.data.code === 2000) {
          ...
          this.uploadImgUrl = data.logo
          this.addShopForm.mct_name = data.merchant_name
          this.addShopForm.mct_abstruct = data.merchant_intro
          this.addShopForm.mct_detail = data.merchant_detail
          this.addShopForm.mct_service = data.merchant_servie
          this.addShopForm.mct_crt_name = data.contract_user
          this.addShopForm.mct_crt_phone = data.contract_phone
          this.addShopForm.mct_crt_address = data.address
        } else {
    ...
    async updateMarchant () {
      try {
        ...
        const res = await updateMarchant(params)
        if (+res.data.code === 2000) {
