    ...
    <div class="add-shop-right-container">
      ...
      <div class="form-box">
        <el-form :model="addShopForm" ref="addShopForm" :rules="addShopFormRules" label-width="150px">
          ...
          <el-form-item>
            <el-button class="add-btn" @click="handleUpdateShop">确定</el-button>
          </el-form-item>
        </el-form>
      </div>
    </div>
...
import { updateGood } from '@/api/becomeSeller.js'
  ...
  methods: {
    ...
    async getGoodsDetail () {
      try {
        ...
        if (+res.data.code === 2000) {
          ...
          const { title, goods_price, goods_name, goods_params, goods_detail, goods_type, mark, goods_img, total_amount, origin_state, serveice, is_admin, is_sale } = res.data.data
          this.addShopForm = Object.assign({}, { title, price: goods_price, goods_name, goods_params, goods_detail, goods_cat_name: goods_name, goods_type_name: goods_type, goods_mark: mark, total_amount, origin_st_name: origin_state, serveice, is_admin, sale: is_sale })
          this.imageUrlList = goods_img.map(item => {
            return {
              id: item.goods_img_id,
              url: item.image_url
            }
          })
        } else {
    ...
    async updateGood () {
      try {
        ...
        const res = await updateGood(params)
        if (+res.data.code === 2000) {
