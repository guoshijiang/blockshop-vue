<template>
<div>
  <div class="forum-child-container">
    <p class="title-box">子版块</p>
    <div
      class="forum-container-item"
      v-for="item in forumChildList"
      :key="item.id"
    >
      <div class="left-forum-box">
        <div class="icon"><i class="el-icon-chat-dot-round"></i></div>
        <div class="name-info-box">
          <p class="name"><router-link :to="{ path: '/forumList', query: { id: item.id }}">{{ item.title }}</router-link></p>
        </div>
      </div>
...
<script>
import bus from '@/bus'
import { getChildForumList, getChildModuleForumList } from '@/api/forum.js'
...
export default {
  ...
  computed: {
    noMore () {
      return this.forumModuleList.length >= +this.forumModuleParams.total
    },
    disabled () {
  ...
  methods: {
    ...
    async getChildForumList () {
      try {
        const res = await getChildForumList(this.forumParams)
        if (+res.data.code === 2000) {
          const data = res.data.data
          console.log(data)
          this.forumChildList = data.form_lst
    ...
    async getChildModuleForumList () {
      try {
        const res = await getChildModuleForumList(this.forumModuleParams)
        if (+res.data.code === 2000) {
          const data = res.data.data
          console.log(data)
          this.forumModuleList = [...this.forumModuleList, ...data.form_lst]
          this.forumModuleParams.total = data.total
        } else {
          this.$message.error(res.msg)
        }
      } catch (e) {
